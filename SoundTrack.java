...
public class SoundTrack {
    ...
    public int method168(int i, int j, int k, int l)
    {
        if(j != 0)
    ...
    public void method169(boolean flag, Buffer buffer0)
    {
        ...
        for(int j = 0; j < 10; j++)
        {
            int k = buffer0.getSmartB();
            if(k == 0)
                break;
            anIntArray106[j] = k;
            anIntArray107[j] = buffer0.getSmartA();
            anIntArray108[j] = buffer0.getSmartB();
        }

        anInt109 = buffer0.getSmartB();
