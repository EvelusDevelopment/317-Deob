...
public class BitmapFont extends BasicRasterizer {
    static boolean test = true;

    public BitmapFont(boolean flag, String s, int junk, FileContainer class44) {
        anInt1485 = 445;
        anInt1486 = -471;
        anInt1487 = -471;
        aBoolean1488 = false;
    ...
    public void draw(byte index[], int x, int y, int w, int h, int color)
    {
        ...
        if(y < BasicRasterizer.heightoffset)
        {
            ...
            i2 += j2 * w;
            offset += j2 * BasicRasterizer.outputwidth;
        }
        ...
        {
            draw(BasicRasterizer.output, index, color, i2, offset, w, h, outincr, indxincr);
            return;
        }
    }
    ...
    public void drawBlended(int i, int x, byte index[], int k, int y, int i1, boolean junk, int alpha) {
        int offset = x + y * BasicRasterizer.outputwidth;
        int l1 = BasicRasterizer.outputwidth - k;
        int i2 = 0;
        int j2 = 0;
        if(y < BasicRasterizer.heightoffset) {
            ...
            y = BasicRasterizer.heightoffset;
            j2 += k2 * k;
            offset += k2 * BasicRasterizer.outputwidth;
        }
        if(y + i1 >= BasicRasterizer.height)
            i1 -= ((y + i1) - BasicRasterizer.height) + 1;
        if(x < BasicRasterizer.widthoffset)
        {
        ...
        if(x + k >= BasicRasterizer.width)
        {
            int i3 = ((x + k) - BasicRasterizer.width) + 1;
            k -= i3;
            i2 += i3;
            l1 += i3;
        }
        if(k <= 0 || i1 <= 0)
            return;
        drawBlended(index, i1, offset, BasicRasterizer.output, j2, k, i2, l1, alpha, i);
    }
